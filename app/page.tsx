...
import { Crown, ArrowUpRight, ShoppingBag, TrendingDown, RefreshCcw } from "lucide-react";
...
export default async function Home() {
  ...
  const processedProducts = products?.map((p) => {
    ...
    const discountRate = (currentWowPrice > 0 && currentWowPrice < currentPrice)
              ...
              <div className="space-y-3 px-2">
                ...
                <div className="flex items-end justify-between pb-2 border-b border-dashed border-gray-200 dark:border-gray-800">
                  <div className="flex flex-col">
                    <span className="text-[10px] font-bold text-muted-foreground tracking-widest uppercase opacity-70 mb-0.5">Price</span>
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-xl font-bold font-mono tracking-tight">
                         {product.hasWowDiscount 
                            ? product.currentWowPrice.toLocaleString() 
                            : (product.currentPrice > 0 ? product.currentPrice.toLocaleString() : "-")
                         }
                      </span>
                      <span className="text-xs font-medium text-muted-foreground">KRW</span>
                    </div>
                  </div>
